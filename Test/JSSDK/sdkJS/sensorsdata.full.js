...
saEvent.send = function(p, callback) {
  ...
  */
  ...
  if (!p.type || p.type.slice(0, 7) !== 'profile') {
    ...
    if(sd.para.preset_properties.latest_wx_ad_click_id && !_.isString(data.properties._latest_wx_ad_click_id)){
  ...
  checkIsFirstLatest: function() {
    ...
    _.each(sd.para.preset_properties,function(value,key){
      ...
      if(value){
        if (key !== 'utm' && url_domain === "url解析失败") {
          ...
          switch (key){
            ...
            case 'landing_page':
              ...
              break;
            case 'wx_ad_click_id':
              var click_id = _.getQueryParam(location.href, 'gdt_vid');
              if(click_id.length){
                latestObj['_latest_wx_ad_click_id'] = (click_id.length == 16 || click_id.length == 18) ? click_id : '参数解析不合法';
              }else{
                latestObj['_latest_wx_ad_click_id'] = '';
              }
              break;
          }
        }
        ...
        }else if(sd.store._state.props && (('_latest_'+key) in sd.store._state.props)){
          delete sd.store._state.props['_latest_'+key];
        }
      }
    });
