...
; (function (root, factory) {
  ...
  // 核心代码快 try catch 捕获报错，不会让页面停止运行
  try {
    ...
    sd.initPara = function (para) {
      ...
      if (_.isObject(sd.para.is_track_latest)) {
        sd.para.is_track_latest = _.extend(sd.para_default.is_track_latest, sd.para.is_track_latest);
      } else {
        sd.para.is_track_latest = sd.para_default.is_track_latest;
      }
    ...
    // 为什么这里要使用自执行函数 里面return 了一个function并希望立即执行
    _.UUID = (function () {
      var T = function () {
    ...
    _.addSinglePageEvent = function (callback) {
      ...
      var singlePageEvent = ('pushState' in window.history ? 'popstate' : 'hashchange');
    ...
    _.getCurrentDomain = function (url) {
      ...
      switch (typeof (sdDomain)) {
        ...
        default:
          if (url === '') {
            return 'url解析失败';
          } else if (_.url("domain", url) === '') {
            return 'url解析失败';
          } else {
            return _.url("domain", url);
          }
      }
    }


    _.getReferrerDomain = function (referrer) {
      var sdDomain = sd.para.referrer_domain;
      switch (typeof (sdDomain)) {
        case "function":
          var resultDomain = sdDomain();
          if (!resultDomain) {
            return _.url("domain", referrer);
          } else if (resultDomain.indexOf(".") !== -1) {
            return resultDomain;
          } else {
            return "referrer解析失败";
    ...
    _.getKeywordFromReferrer = function () {
      ...
      if (document && typeof document.referrer === 'string') {
        if (document.referrer.indexOf('http') === 0) {
          var domain = _.getReferrerDomain(document.referrer);
          var query = _.url('?', document.referrer);
          var temp = null;
          for (var i in search_keyword) {
            if (domain.split && domain.split('.')[0] === i) {
    ...
    _.info = {
      initPage: function () {
        ...
        var referrer_domain = _.getReferrerDomain(referrer);
    ...
    var saNewUser = {
      ...
      checkIsFirstLatest: function () {
        ...
        _.each(sd.para.is_track_latest, function (value, key) {
          if (value) {
            if (key !== 'utm' && url_domain === "url解析失败") {
              latestObj['$latest_' + key] = 'url的domain解析失败';
            } else if (key !== 'utm' && referrer_domain === 'referrer解析失败') {
              latestObj['$latest_' + key] = 'referrer的domain解析失败';
            } else if (url_domain !== referrer_domain) {
              switch (key) {
                ...
                case 'referrer':
                  ...
                  break;
                case 'referrer_host':
                  latestObj['$latest_referrer_host'] = _.info.pageProp.referrer_host;
                  break;
                case 'search_keyword':
        ...
        // utm
        if (sd.para.is_track_latest.utm) {
    ...
    saEvent.send = function (p, callback) {
      ...
      if (!p.type || p.type.slice(0, 7) !== 'profile') {
        ...
        if (sd.para.is_track_latest.referrer && !_.isString(data.properties.$latest_referrer)) {
          data.properties.$latest_referrer = '取值异常';
          _.jssdkDebug(data.properties, store.getProps());
        }
        if (sd.para.is_track_latest.referrer_host && !_.isString(data.properties.$latest_referrer_host)) {
          data.properties.$latest_referrer_host = '取值异常';
        }
        if (sd.para.is_track_latest.search_keyword && !_.isString(data.properties.$latest_search_keyword)) {
          data.properties.$latest_search_keyword = '取值异常';
        }
        if (sd.para.is_track_latest.traffic_source_type && !_.isString(data.properties.$latest_traffic_source_type)) {
    ...
    var commonWays = {
      ...
      trackHeatMap: function (target, callback) {
        if ((typeof target === 'object') && target.tagName) {
          var tagName = target.tagName.toLowerCase();
          var parent_ele = target.parentNode.tagName.toLowerCase();
          if (tagName !== 'button' && tagName !== 'a' && parent_ele !== 'a' && parent_ele !== 'button' && tagName !== 'input' && tagName !== 'textarea') {
            heatmap.start(null, target, tagName, callback);
          }
        }
      },
      trackAllHeatMap: function (target, callback) {
        if ((typeof target === 'object') && target.tagName) {
          var tagName = target.tagName.toLowerCase();
          heatmap.start(null, target, tagName, callback);
        }
      ...
      autoTrackSinglePage: function (para, callback) {
        ...
        function closure(p, c) {
          sd.track('$pageview', _.extend({
            $referrer: url,
            $referrer_host: url ? _.url('hostname', url) : url,
            $url: location.href,
    ...
    var heatmap = {
      ...
      selector: function (el) {
        ...
        if (el.getAttribute && el.getAttribute('id') && sd.para.heatmap.element_selector !== 'not_use_id') {
      ...
      getDomSelector: function (el, arr) {
        ...
        if (el.getAttribute && el.getAttribute('id')) return arr.join(' > ');
      ...
      start: function (ev, target, tagName, callback) {
        ...
        if (tagName === 'a' && sd.para.heatmap && sd.para.heatmap.isTrackLink === true) {
          ...
          sd.track('$WebClick', prop, callback);
        }
      ...
      initScrollmap: function () {
        ...
        var delayTime = interDelay({
          timeout: 1000,
          func: function (para) {
            var offsetTop = document.documentElement.scrollTop || window.pageYOffset || document.body.scrollTop || 0;
            var current_time = new Date();
            var delay_time = current_time - this.current_time;
            if (delay_time > sd.para.heatmap.scroll_delay_time && offsetTop - para.$viewport_position !== 0) {
    ...
    sd.init = function (para) {
      // 标志已经init过
      if (sd.readyState && sd.readyState >= 2) {
        return false;
      }
      // 如果有window['sensorsDataAnalytic201505']声明，且没有参数传入，那就是异步模式
      // 如果没有window['sensorsDataAnalytic201505']声明，且有参数传入，表示是module模式
        ...
        if (sd.para.is_track_single_page) {
          _.addSinglePageEvent(function (last_url) {
            if (last_url !== location.href) {
              sd.quick("autoTrack", { $url: location.href, $referrer: last_url });
            }
          });
        }
        // 初始化distinct_id
